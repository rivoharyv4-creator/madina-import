...
<body>
    ...
    <table class="meta">
        <tr>
            ...
            <td><div class="box box-right"><div class="label">Informations du document</div><div style="margin-top:5px"><strong>Date :</strong> {{ $module==='devis' ? \Carbon\Carbon::parse($document->created_at,config('app.timezone'))->timezone(config('madina.company.timezone','Asia/Shanghai'))->format('d/m/Y') : \Carbon\Carbon::parse($document->issued_at)->format('d/m/Y') }}</div>@if($module==='devis')<div><strong>Valide jusqu’au :</strong> {{ \Carbon\Carbon::parse($document->valid_until)->format('d/m/Y') }}</div><div><strong>Mode d’envoi :</strong> {{ ucfirst($document->shipping_mode?:'Non défini') }}</div>@else<div><strong>Commande :</strong> {{ $document->order_number }}</div><div class="status">{{ str_replace('_',' ',$document->status) }}</div>@endif</div></td>
        </tr>
    </table>
