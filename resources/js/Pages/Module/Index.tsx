...
export default function Index({module,config,rows,pagination,query,filterOptions=[],activeFilters={}}:{module:string;config:any;rows:any[];pagination:Pagination;query?:string;filterOptions?:FilterOption[];activeFilters?:Record<string,string>}){
 ...
 >
  ...
  <section className="panel overflow-hidden !p-0">
   ...
   <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr>{Object.values(config.columns).map((column:any)=><th key={column}>{column}</th>)}{(config.editable||config.viewable)&&<th className="w-36 text-center">Actions</th>}</tr></thead><tbody>{rows.map((row:any)=><tr key={row.id}>{Object.keys(config.columns).map(key=><td key={key}>{key==='photo_path'?(row[key]?<img src={`/product-photo/${String(row[key]).split('/').pop()}`} alt={row.name||'Produit'} className="size-12 rounded-xl border border-gray-200 object-cover"/>:<span className="text-gray-300">Aucune</span>):key==='proof_path'?(row[key]?<a href={row[key]} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-[#FCF108]/20 px-2.5 py-1.5 text-xs font-semibold text-[#665f00] hover:bg-[#FCF108]/35"><ExternalLink size={13}/>Voir</a>:<span className="text-gray-300">Aucun</span>):key==='quality_rating'?<span className="inline-flex items-center gap-0.5" title={`${row[key]} sur 5`}>{[1,2,3,4,5].map(star=><Star key={star} size={15} className={star<=Number(row[key])?'fill-[#FCF108] text-[#D5C900]':'fill-gray-50 text-gray-200'}/>)}</span>:<span className={['status','active','is_published'].includes(key)?'status':''}>{display(key,row[key])}</span>}</td>)}{(config.editable||config.viewable)&&<td><div className="flex justify-center gap-2">{module==='demandes-devis'&&(row.quote_record_id?<Link href={`/modules/devis/${row.quote_record_id}/edit`} title="Ouvrir le devis lié" aria-label="Ouvrir le devis lié" className="grid size-9 place-items-center rounded-lg border border-emerald-200 text-emerald-600 transition hover:bg-emerald-50"><Eye size={16}/></Link>:<Link href={`/modules/devis/create?quote_request_id=${row.id}`} title="Créer un devis" aria-label="Créer un devis" className="grid size-9 place-items-center rounded-lg border border-[#BD2433]/30 text-[#BD2433] transition hover:bg-red-50"><FilePlus2 size={16}/></Link>)}{['commandes','logistique','demandes'].includes(module)&&<Link href={`/modules/${module}/${row.id}`} title={module==='logistique'?'Voir tous les détails':module==='demandes'?'Voir la demande':'Voir l’aperçu'} aria-label={module==='logistique'?'Voir tous les détails':module==='demandes'?'Voir la demande':'Voir l’aperçu'} className="grid size-9 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-[#FCF108] hover:bg-yellow-50 hover:text-[#817900]"><Eye size={16}/></Link>}{['devis','factures'].includes(module)&&<a href={`/modules/${module}/${row.id}/pdf`} title="Télécharger le PDF" aria-label="Télécharger le PDF" className="grid size-9 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-[#FCF108] hover:bg-yellow-50 hover:text-[#817900]"><FileDown size={16}/></a>}{module==='paiements'&&<a href={`/modules/paiements/${row.id}/recu`} title="Télécharger le reçu de paiement" aria-label="Télécharger le reçu de paiement" className="grid size-9 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-[#FCF108] hover:bg-yellow-50 hover:text-[#817900]"><FileDown size={16}/></a>}{config.editable&&<Link href={`/modules/${module}/${row.id}/edit`} title="Modifier" aria-label="Modifier" className="grid size-9 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-[#BD2433]/30 hover:bg-red-50 hover:text-[#BD2433]"><Pencil size={15}/></Link>}</div></td>}</tr>)}</tbody></table>
