...
class StoreModuleRequest extends FormRequest
{
    protected function prepareForValidation(): void
    {
        ...
        if ($this->route('module') === 'commandes') {
            ...
            if (isset($legacy[$this->input('status')])) {
                $this->merge(['status' => $legacy[$this->input('status')]]);
            }
            $items = collect($this->input('items', []))->map(function ($item) {
                $quantity = (float) ($item['quantity'] ?? 0);
                if (empty($item['inventory_product_id'])) {
                    $unitPrice = (float) ($item['supplier_price'] ?? 0)
                        + (float) ($item['china_delivery'] ?? 0)
                        + (float) ($item['packaging'] ?? 0)
                        + (float) ($item['freight'] ?? 0)
                        + (float) ($item['margin'] ?? 0);

                    return [...$item, 'client_total' => $unitPrice * $quantity];
                }
                $product = DB::table('inventory_products')->whereNull('deleted_at')->find($item['inventory_product_id']);
                if (! $product) {
                    return $item;
                }
                ...
                return [...$item,
                    ...
                    'supplier_price' => $product->purchase_price,
                    'china_delivery' => 0,
                    'packaging' => 0,
                    'weight' => null,
                    'cbm' => $product->cbm,
                    'freight' => 0,
                    'margin' => max(0, $total - (float) $product->purchase_price * $quantity),
                    'commission' => 0,
                    'client_total' => $total,
                ];
            })->all();
        ...
        if ($this->route('module') === 'paiements') {
            ...
            if (isset($legacyTypes[$reason])) {
                $this->merge(['payment_reason' => $reason, 'type' => $legacyTypes[$reason]]);
            }
        }
        if ($this->route('module') === 'achats' && $this->filled('quantity') && $this->filled('unit_price')) {
            $this->merge(['amount' => (float) $this->input('quantity') * (float) $this->input('unit_price')]);
        }
    }
    ...
    public function rules(): array
    {
        ...
        return match ($this->route('module')) {
            ...
            'stock' => ['reference' => ['nullable', 'string', 'max:100', Rule::unique('inventory_products', 'reference')->ignore($this->route('id'))], 'name' => ['required', 'string', 'max:255'], 'slug' => ['nullable', 'string', 'max:255', Rule::unique('inventory_products', 'slug')->ignore($this->route('id'))], 'photo' => $photo, 'gallery' => ['nullable', 'array', 'max:6'], 'gallery.*' => $photo, 'quantity' => ['required', 'integer', 'min:0'], 'reserved_quantity' => ['nullable', 'integer', 'min:0', 'lte:quantity'], 'purchase_price' => $money, 'cbm' => ['nullable', 'numeric', 'min:0'], 'freight' => $optionalMoney, 'sale_price' => $money, 'origin_order_ids' => ['nullable', 'array'], 'origin_order_ids.*' => ['integer', 'distinct', 'exists:orders,id'], 'alert_threshold' => ['nullable', 'integer', 'min:0'], 'is_published' => ['boolean'], 'is_featured' => ['boolean'], 'show_price' => ['boolean'], 'category' => ['nullable', 'string', 'max:120'], 'short_description' => ['nullable', 'string', 'max:320'], 'catalog_description' => ['nullable', 'string', 'max:5000']],
            'catalogue' => ['slug' => ['nullable', 'string', 'max:255', Rule::unique('inventory_products', 'slug')->ignore($this->route('id'))], 'gallery' => ['nullable', 'array', 'max:6'], 'gallery.*' => $photo, 'is_published' => ['required', 'boolean'], 'public_availability_status' => ['required', 'in:out_of_stock,available_now,on_order'], 'is_featured' => ['required', 'boolean'], 'show_price' => ['required', 'boolean'], 'category' => ['nullable', 'string', 'max:120'], 'short_description' => ['nullable', 'string', 'max:320'], 'catalog_description' => ['nullable', 'string', 'max:5000']],
            'demandes-devis' => ['client_name' => ['required', 'string', 'max:255'], 'client_contact' => ['required', 'string', 'max:255'], 'source' => ['required', 'in:facebook,bouche_a_oreille,autre'], 'description' => ['required', 'string', 'max:5000'], 'quantity' => ['required', 'integer', 'min:1'], 'budget' => $optionalMoney, 'desired_deadline' => ['nullable', 'string', 'max:255'], 'shipping_mode' => ['required', 'in:maritime,aerien,non_precise'], 'status' => ['required', 'in:nouveau,a_contacter,a_qualifier,qualifie,devis_envoye,en_negociation,commande_confirmee,perdu,en_attente'], 'sourcing_priority' => ['required', 'in:urgent,prioritaire,normal,faible'], 'destination' => ['nullable', 'string', 'max:255'], 'request_date' => ['required', 'date'], 'assigned_to' => ['nullable', 'exists:users,id'], 'internal_note' => ['nullable', 'string']],
            ...
            'factures' => ['order_id' => ['required', 'exists:orders,id'], 'type' => ['required', 'in:produits,frais'], 'issued_at' => ['required', 'date'], 'subtotal' => $money, 'paid_amount' => $optionalMoney, 'status' => ['required', 'in:brouillon,provisoire,finale,payee,partielle,annulee'], 'shipping_delay' => ['nullable', 'string', 'max:255'], 'bank_details' => ['nullable', 'string'], 'payment_terms' => ['nullable', 'string'], 'warranty' => ['nullable', 'string'], 'notes' => ['nullable', 'string']],
            'achats' => ['supplier_id' => ['required', 'exists:suppliers,id'], 'order_id' => ['required', 'exists:orders,id'], 'paid_at' => ['required', 'date'], 'quantity' => ['required', 'numeric', 'gt:0'], 'unit_price' => array_merge($money, ['gt:0']), 'amount' => array_merge($money, ['gt:0']), 'method' => ['required', 'in:WeChat,Alipay,banque'], 'reference' => ['nullable', 'string', 'max:255'], 'proof' => $photo, 'proof_url' => ['nullable', 'url', 'max:2000'], 'status' => ['required', 'in:paye,partiel,en_attente'], 'notes' => ['nullable', 'string']],
            ...
            'fiscalite' => ['fiscal_year' => ['required', 'integer', 'between:2020,2100'], 'declared_amount' => $optionalMoney, 'due_at' => ['nullable', 'date'], 'status' => ['required', 'in:estimation,a_declarer,declare,paye']],
            default => abort(404),
        };
    }
