...
class ModuleController extends Controller
{
    ...
    public function index(Request $request, string $module)
    {
        ...
        if (DB::getSchemaBuilder()->hasTable($config['table'])) {
            ...
            if ($module === 'devis') {
                $rows = array_map(fn ($row) => [...$row, 'created_at' => $this->chinaDate($row['created_at'])], $rows);
            }
    ...
    public function edit(string $module, int $id)
    {
        ...
        if (DB::getSchemaBuilder()->hasColumn($config['table'], 'deleted_at')) {
            $query->whereNull('deleted_at');
        } $record = $query->find($id);
        abort_unless($record, 404);
        $values = $this->editValues($module, $record);
        $fields = array_map(function ($field) use ($values) {
            if (array_key_exists($field['name'], $values)) {
                $field['default'] = $values[$field['name']];
            } if ($field['type'] === 'file' && ! empty($values['photo_path'])) {
                $field['preview'] = '/product-photo/'.basename($values['photo_path']);
            } if ($field['name'] === 'proof' && ! empty($values['proof_path'])) {
                $field['preview'] = '/purchase-proof/'.basename($values['proof_path']);
            }
    ...
    {
        if ($module === 'demandes') {
            return $this->showPublicRequest($id);
        }
        if ($module === 'logistique') {
            return $this->showShipment($id);
        }
    ...
    private function showPublicRequest(int $id)
    {
        ...
        if ($contactRequest->status === 'nouvelle') {
            DB::table('contact_requests')->where('id', $id)->update(['status' => 'lue', 'updated_at' => now()]);
    ...
    public function pdf(string $module, int $id)
    {
        ...
        if ($module === 'devis') {
            ...
            $items = DB::table('quote_items')->where('quote_id', $id)->orderBy('id')->get()->map(function ($item) {
                $item->photo_data = $this->storage->dataUri($item->photo_path);
    ...
    {
        $employee = DB::table('employees')->whereNull('deleted_at')->find($id);
        abort_unless($employee, 404);
        DB::transaction(function () use ($request, $employee, $id) {
            $leftAt = $request->input('left_at', today()->toDateString());
            $reason = $request->input('departure_reason') ?: 'Départ de l’entreprise';
            DB::table('employees')->where('id', $id)->update(['active' => false, 'left_at' => $leftAt, 'departure_reason' => $reason, 'deleted_at' => now(), 'updated_at' => now()]);
            DB::table('audit_logs')->insert(['user_id' => $request->user()->id, 'event' => 'employes.depart', 'auditable_type' => 'employees', 'auditable_id' => $id, 'old_values' => $this->auditJson($employee), 'new_values' => $this->auditJson(['active' => false, 'left_at' => $leftAt, 'departure_reason' => $reason]), 'ip_address' => $request->ip(), 'created_at' => now()]);
        });
    ...
    public function store(StoreModuleRequest $request, string $module, NumberSequenceService $numbers, BusinessCalculator $calculator)
    {
        ...
        $result = DB::transaction(function () use ($module, $data, $request, $numbers, $calculator) {
            ...
            if ($module === 'devis' && ! empty($data['quote_request_id'])) {
                DB::table('quote_requests')->where('id', $data['quote_request_id'])->update(['quote_id' => $id, 'status' => 'devis_envoye', 'updated_at' => $now]);
            }
    ...
    private function createStock(array $data, NumberSequenceService $numbers, int $userId): int
    {
        ...
        if ($quantity > 0) {
            DB::table('stock_movements')->insert(['inventory_product_id' => $id, 'type' => 'entree', 'quantity' => $quantity, 'before_quantity' => 0, 'after_quantity' => $quantity, 'notes' => 'Stock initial', 'moved_at' => now(), 'user_id' => $userId]);
        }
    ...
    private function updateStock(int $id, object $old, array $data, int $userId): void
    {
        ...
        $this->syncStockOrigins($id, $origins);
        if ($after !== $before) {
            DB::table('stock_movements')->insert(['inventory_product_id' => $id, 'type' => 'inventaire', 'quantity' => abs($after - $before), 'before_quantity' => $before, 'after_quantity' => $after, 'notes' => 'Correction manuelle auditée', 'moved_at' => now(), 'user_id' => $userId]);
        }
    }
    ...
    private function syncStockOrigins(int $productId, array $orderIds): void
    {
        ...
        foreach (array_unique($orderIds) as $orderId) {
            DB::table('inventory_product_origins')->insert(['inventory_product_id' => $productId, 'order_id' => $orderId, 'created_at' => $now, 'updated_at' => $now]);
        }
    }
    ...
    private function createQuote(array $data, NumberSequenceService $numbers): int
    {
        if (empty($data['client_id'])) {
            $data['client_id'] = DB::table('clients')->insertGetId(['number' => $numbers->next('client'), 'name' => $data['client_name'], 'contact' => $data['client_contact'], 'type' => $data['client_type'], 'address' => null, 'notes' => 'Créé directement depuis un devis', 'active' => true, 'credit_balance' => 0, 'created_at' => now(), 'updated_at' => now()]);
        }
        $client = DB::table('clients')->find($data['client_id']);
        $supplierTotal = collect($data['items'])->sum(fn ($item) => (float) $item['supplier_price'] * (float) $item['quantity']);
        $logistics = collect($data['items'])->sum(fn ($item) => ((float) ($item['china_delivery'] ?? 0) + (float) ($item['packaging'] ?? 0) + (float) ($item['freight'] ?? 0)) * (float) $item['quantity']);
        $margin = collect($data['items'])->sum(fn ($item) => (float) ($item['margin'] ?? 0) * (float) $item['quantity']);
        $total = collect($data['items'])->sum(fn ($item) => $this->quoteItemSalePrice($item));
    ...
    private function createOrder(array $data, NumberSequenceService $numbers, int $managerId): int
    {
        if (! empty($data['new_client_name'])) {
            $data['client_id'] = DB::table('clients')->insertGetId(['number' => $numbers->next('client'), 'name' => $data['new_client_name'], 'contact' => $data['new_client_contact'], 'type' => $data['new_client_type'], 'address' => $data['new_client_address'] ?? null, 'notes' => 'Créé directement depuis une commande', 'active' => true, 'credit_balance' => 0, 'created_at' => now(), 'updated_at' => now()]);
        }
        $total = collect($data['items'])->sum(fn ($item) => (float) $item['client_total']);
        $deposit = (float) ($data['deposit'] ?? 0);
        if ($deposit > $total) {
            throw ValidationException::withMessages(['deposit' => 'L’acompte ne peut pas dépasser le total client.']);
        } $supplierTotal = collect($data['items'])->sum(fn ($item) => (float) $item['supplier_price'] * (float) $item['quantity']);
        $freight = collect($data['items'])->sum(fn ($item) => (float) ($item['freight'] ?? 0));
        $margin = collect($data['items'])->sum(fn ($item) => (float) ($item['margin'] ?? 0));
        $cbm = collect($data['packages'] ?? [])->sum(fn ($package) => (float) ($package['volume_cbm'] ?? 0)) ?: collect($data['items'])->sum(fn ($item) => (float) ($item['cbm'] ?? 0));
        $commissionEnabled = (bool) ($data['commission_enabled'] ?? false);
        $commission = $commissionEnabled ? collect($data['items'])->sum(fn ($item) => (float) ($item['commission'] ?? 0)) : 0;
    ...
    private function insertQuoteItems(int $quoteId, array $items, array $existingPhotos = []): void
    {
        foreach ($items as $item) {
            ...
            $existing = $existingPhotos[$item['id'] ?? 0] ?? null;
    ...
    private function insertOrderItems(int $orderId, array $items, bool $commissionEnabled, string $status, array $existingPhotos = []): array
    {
        ...
        foreach ($items as $item) {
            $photo = $item['photo'] ?? null;
            $quotePhoto = ! empty($item['quote_item_id']) ? DB::table('quote_items')->where('id', $item['quote_item_id'])->value('photo_path') : null;
            $existing = $existingPhotos[$item['id'] ?? 0] ?? $quotePhoto;
            $supplier = ! empty($item['supplier_id']) ? DB::table('suppliers')->find($item['supplier_id']) : null;
    ...
    private function insertPackages(int $orderId, array $packages, array $itemIds, array $items): void
    {
        ...
        foreach ($packages as $packageIndex => $package) {
            ...
            if (isset($references[$normalizedReference])) {
                throw ValidationException::withMessages(["packages.$packageIndex.reference" => 'Chaque colis doit avoir une référence différente.']);
            }
            $references[$normalizedReference] = true;
            $seen = [];
            ...
            foreach ($package['items'] as $lineIndex => $line) {
                ...
                if (! isset($itemIds[$index])) {
                    throw ValidationException::withMessages(["packages.$packageIndex.items.$lineIndex.item_index" => 'Produit sélectionné invalide.']);
                }
                if (isset($seen[$index])) {
                    throw ValidationException::withMessages(["packages.$packageIndex.items.$lineIndex.item_index" => 'Ce produit est déjà présent dans ce colis.']);
                }
                $seen[$index] = true;
                $allocated[$index] = ($allocated[$index] ?? 0) + (float) $line['quantity'];
                if ($allocated[$index] > (float) $items[$index]['quantity']) {
                    throw ValidationException::withMessages(["packages.$packageIndex.items.$lineIndex.quantity" => 'La quantité répartie dans les colis dépasse la quantité commandée.']);
                }
    ...
    {
        if (! empty($data['new_client_name'])) {
            $data['client_id'] = DB::table('clients')->insertGetId(['number' => $numbers->next('client'), 'name' => $data['new_client_name'], 'contact' => $data['new_client_contact'], 'type' => $data['new_client_type'], 'address' => $data['new_client_address'] ?? null, 'notes' => 'Créé directement depuis un paiement', 'active' => true, 'credit_balance' => 0, 'created_at' => now(), 'updated_at' => now()]);
        }
        ...
        $allocated = $this->paymentAllocation($data);
        $id = DB::table('client_payments')->insertGetId([...$data, 'allocated_amount' => $allocated, 'status' => 'valide', 'created_at' => now(), 'updated_at' => now()]);
        if ($amount > $allocated) {
            DB::table('clients')->where('id', $data['client_id'])->increment('credit_balance', $amount - $allocated);
        }
        if (! empty($data['order_id'])) {
            $order = DB::table('orders')->lockForUpdate()->find($data['order_id']);
            DB::table('orders')->where('id', $order->id)->update(['deposit' => (float) $order->deposit + $allocated, 'balance_due' => max(0, (float) $order->balance_due - $allocated), 'updated_at' => now()]);
        }
        if (! empty($data['invoice_id'])) {
            $invoice = DB::table('invoices')->lockForUpdate()->find($data['invoice_id']);
            $paid = (float) $invoice->paid_amount + $allocated;
            DB::table('invoices')->where('id', $invoice->id)->update(['paid_amount' => $paid, 'balance_due' => max(0, (float) $invoice->subtotal - $paid), 'status' => $paid >= (float) $invoice->subtotal ? 'payee' : 'partielle', 'updated_at' => now()]);
        }
    ...
    private function createSupplierPayment(array $data): int
    {
        ...
        $id = DB::table('supplier_payments')->insertGetId([...Arr::except($data, ['order_id', 'proof']), 'proof_path' => $this->storePurchaseProof($proof), 'created_at' => now(), 'updated_at' => now()]);
        DB::table('supplier_payment_allocations')->insert(['supplier_payment_id' => $id, 'order_id' => $data['order_id'], 'amount' => $data['amount']]);
    ...
    {
        $product = DB::table('inventory_products')->lockForUpdate()->find($data['inventory_product_id']);
        $quantity = (float) $data['quantity'];
        if ($quantity > (float) $product->available_quantity) {
            throw ValidationException::withMessages(['quantity' => 'Stock disponible insuffisant. Quantité disponible : '.$product->available_quantity]);
        }
        $total = $quantity * (float) $data['unit_price'];
        $paid = (float) ($data['paid_amount'] ?? 0);
        if ($paid > $total) {
            throw ValidationException::withMessages(['paid_amount' => 'Le paiement ne peut pas dépasser le total de la vente.']);
        } if ($paid < $total && empty($data['buyer_name'])) {
            throw ValidationException::withMessages(['buyer_name' => 'Le nom de l’acheteur est obligatoire pour une vente non soldée.']);
        } if ($paid < $total && empty($data['buyer_contact'])) {
            throw ValidationException::withMessages(['buyer_contact' => 'Le contact est obligatoire pour une vente non soldée.']);
        }
        $after = (float) $product->quantity - $quantity;
        DB::table('inventory_products')->where('id', $product->id)->update(['quantity' => $after, 'available_quantity' => max(0, $after - (float) $product->reserved_quantity), 'total_purchase_cost' => $after * (float) $product->purchase_price + (float) $product->freight, 'sale_total' => $after * (float) $product->sale_price, 'stock_value' => $after * (float) $product->purchase_price, 'exited_at' => today(), 'updated_at' => now()]);
        DB::table('stock_movements')->insert(['inventory_product_id' => $product->id, 'type' => 'sortie', 'quantity' => $quantity, 'before_quantity' => $product->quantity, 'after_quantity' => $after, 'notes' => 'Vente locale', 'moved_at' => now(), 'user_id' => $userId]);
    ...
    {
        $calc = $calculator->salary($data['gross_salary'], $data['irsa_mode'], $data['irsa_value'] ?? 0);
        $id = DB::table('salaries')->insertGetId(['employee_id' => $data['employee_id'], 'month' => date('Y-m-01', strtotime($data['month'])), 'gross_salary' => $data['gross_salary'], 'irsa_mode' => $data['irsa_mode'], 'irsa_rate' => $data['irsa_mode'] === 'pourcentage' ? $data['irsa_value'] : null, 'irsa_amount' => $calc['irsa'], 'net_salary' => $calc['net'], 'paid_at' => $data['paid_at'] ?? null, 'status' => $data['status'], 'created_at' => now(), 'updated_at' => now()]);
        if ($data['status'] === 'paye') {
            DB::table('expenses')->insert(['category' => 'salaire', 'amount' => $calc['net'], 'spent_at' => $data['paid_at'] ?? today(), 'type' => 'business', 'description' => 'Salaire net automatique', 'order_id' => null, 'status' => 'paye', 'source_type' => 'salary', 'source_id' => $id, 'created_at' => now(), 'updated_at' => now()]);
            DB::table('expenses')->insert(['category' => 'IRSA', 'amount' => $calc['irsa'], 'spent_at' => $data['paid_at'] ?? today(), 'type' => 'business', 'description' => 'IRSA automatique', 'order_id' => null, 'status' => 'paye', 'source_type' => 'salary_irsa', 'source_id' => $id, 'created_at' => now(), 'updated_at' => now()]);
        }
    ...
    private function editValues(string $module, object $record): array
    {
        $values = (array) $record;
        if ($module === 'devis') {
            $values['client_contact'] = $record->contact;
            $item = DB::table('quote_items')->where('quote_id', $record->id)->first();
            if ($item) {
                $values = [...$values, 'product_name' => $item->name, 'specifications' => $item->specifications, 'quantity' => $item->quantity, 'supplier_id' => $item->supplier_id, 'commission' => $item->commission];
            }
        }
        if ($module === 'commandes') {
            $item = DB::table('order_items')->where('order_id', $record->id)->first();
            if ($item) {
                $values = [...$values, 'product_name' => $item->name, 'specifications' => $item->specifications, 'quantity' => $item->quantity, 'supplier_id' => $item->supplier_id, 'supplier_price' => $record->supplier_total];
            }
        }
        if ($module === 'achats') {
        ...
        if ($module === 'paiements') {
            [$values['type'],$values['payment_object'],$values['notes']] = $this->paymentReasonAndNotes($record->type, $record->notes);
        }
        if ($module === 'salaires') {
            $values['irsa_value'] = $record->irsa_mode === 'pourcentage' ? $record->irsa_rate : $record->irsa_amount;
            $values['month'] = date('Y-m', strtotime($record->month));
        }
    ...
    private function applyUpdate(string $module, int $id, object $old, array $data, int $userId, BusinessCalculator $calculator): void
    {
        if ($module === 'fournisseurs') {
            $products = $data['products'] ?? [];
            DB::table('suppliers')->where('id', $id)->update([...Arr::except($data, 'products'), 'updated_at' => now()]);
            $this->replaceSupplierProducts($id, $products);

            return;
        }
        if (in_array($module, ['clients', 'demandes-devis', 'logistique', 'depenses', 'employes'], true)) {
            DB::table($this->config($module)['table'])->where('id', $id)->update([...$data, 'updated_at' => now()]);

            return;
        }
        if ($module === 'stock') {
            $this->updateStock($id, $old, $data, $userId);

            return;
        }
        if ($module === 'catalogue') {
            $this->updateCatalogue($id, $old, $data);
        ...
        if ($module === 'devis') {
            ...
            $photos = DB::table('quote_items')->where('quote_id', $id)->pluck('photo_path', 'id')->all();
            DB::table('quote_items')->where('quote_id', $id)->delete();
            $this->insertQuoteItems($id, $data['items'], $photos);
        ...
        if ($module === 'commandes') {
            ...
            $margin = collect($data['items'])->sum(fn ($item) => (float) ($item['margin'] ?? 0));
            $cbm = collect($data['packages'] ?? [])->sum(fn ($package) => (float) ($package['volume_cbm'] ?? 0)) ?: collect($data['items'])->sum(fn ($item) => (float) ($item['cbm'] ?? 0));
            $enabled = (bool) ($data['commission_enabled'] ?? false);
            $commission = $enabled ? collect($data['items'])->sum(fn ($item) => (float) ($item['commission'] ?? 0)) : 0;
            ...
            $photos = $oldItems->pluck('photo_path', 'id')->all();
            DB::table('order_packages')->where('order_id', $id)->delete();
            DB::table('order_items')->where('order_id', $id)->delete();
            $itemIds = $this->insertOrderItems($id, $data['items'], $enabled, $data['status'], $photos);
            $this->insertPackages($id, $data['packages'] ?? [], $itemIds, $data['items']);
        ...
        if ($module === 'paiements') {
            ...
            $amount = (float) $data['amount'];
            $allocated = $this->paymentAllocation($data);
            $this->reversePaymentEffects($old);
            DB::table('client_payments')->where('id', $id)->update([...$data, 'allocated_amount' => $allocated, 'status' => 'valide', 'updated_at' => now()]);
            $this->applyPaymentEffects((object) [...$data, 'amount' => $amount, 'allocated_amount' => $allocated]);
        ...
        if ($module === 'achats') {
            $proof = $data['proof'] ?? null;
            DB::table('supplier_payments')->where('id', $id)->update([...Arr::except($data, ['order_id', 'proof']), 'proof_path' => $proof ? $this->storePurchaseProof($proof) : $old->proof_path, 'updated_at' => now()]);
            DB::table('supplier_payment_allocations')->where('supplier_payment_id', $id)->update(['order_id' => $data['order_id'], 'amount' => $data['amount']]);

            return;
        }
        if ($module === 'ventes') {
            $this->updateLocalSale($id, $old, $data, $userId);

            return;
        }
        if ($module === 'salaires') {
            $calc = $calculator->salary($data['gross_salary'], $data['irsa_mode'], $data['irsa_value'] ?? 0);
            DB::table('salaries')->where('id', $id)->update(['employee_id' => $data['employee_id'], 'month' => date('Y-m-01', strtotime($data['month'])), 'gross_salary' => $data['gross_salary'], 'irsa_mode' => $data['irsa_mode'], 'irsa_rate' => $data['irsa_mode'] === 'pourcentage' ? $data['irsa_value'] : null, 'irsa_amount' => $calc['irsa'], 'net_salary' => $calc['net'], 'paid_at' => $data['paid_at'] ?? null, 'status' => $data['status'], 'updated_at' => now()]);
            DB::table('expenses')->where('source_type', 'salary')->where('source_id', $id)->update(['amount' => $calc['net'], 'spent_at' => $data['paid_at'] ?? today(), 'updated_at' => now()]);
            DB::table('expenses')->where('source_type', 'salary_irsa')->where('source_id', $id)->update(['amount' => $calc['irsa'], 'spent_at' => $data['paid_at'] ?? today(), 'updated_at' => now()]);
    ...
    private function syncOrderStock(iterable $oldItems, bool $oldApplied, array $newItems, bool $newApplied, int $userId, string $orderNumber): void
    {
        ...
        foreach ($productIds as $productId) {
            ...
            if (! $product) {
                throw ValidationException::withMessages(['items' => 'Un produit de stock sélectionné n’est plus disponible.']);
            }
    ...
    {
        $unallocated = max(0, (float) $payment->amount - (float) $payment->allocated_amount);
        $client = DB::table('clients')->lockForUpdate()->find($payment->client_id);
        DB::table('clients')->where('id', $client->id)->update(['credit_balance' => max(0, (float) $client->credit_balance - $unallocated), 'updated_at' => now()]);
        if ($payment->order_id) {
            $order = DB::table('orders')->lockForUpdate()->find($payment->order_id);
            DB::table('orders')->where('id', $order->id)->update(['deposit' => max(0, (float) $order->deposit - (float) $payment->allocated_amount), 'balance_due' => min((float) $order->client_total, (float) $order->balance_due + (float) $payment->allocated_amount), 'updated_at' => now()]);
        }
        if ($payment->invoice_id) {
            $invoice = DB::table('invoices')->lockForUpdate()->find($payment->invoice_id);
            $paid = max(0, (float) $invoice->paid_amount - (float) $payment->allocated_amount);
            DB::table('invoices')->where('id', $invoice->id)->update(['paid_amount' => $paid, 'balance_due' => (float) $invoice->subtotal - $paid, 'status' => $paid > 0 ? 'partielle' : 'finale', 'updated_at' => now()]);
        }
    }
    ...
    {
        $amount = (float) $data['amount'];
        if (empty($data['order_id']) && empty($data['invoice_id'])) {
            return 0;
        }
        $allocated = (float) ($data['allocated_amount'] ?? 0);
        if ($allocated > $amount) {
            throw ValidationException::withMessages(['allocated_amount' => 'Le montant affecté ne peut pas dépasser le montant reçu.']);
        }
        if (! empty($data['order_id']) && DB::table('orders')->where('id', $data['order_id'])->value('client_id') != $data['client_id']) {
            throw ValidationException::withMessages(['order_id' => 'Cette commande n’appartient pas au client sélectionné.']);
        }
        if (! empty($data['invoice_id']) && DB::table('invoices')->where('id', $data['invoice_id'])->value('client_id') != $data['client_id']) {
            throw ValidationException::withMessages(['invoice_id' => 'Cette facture n’appartient pas au client sélectionné.']);
        }
    ...
    private function applyPaymentEffects(object $payment): void
    {
        $unallocated = max(0, (float) $payment->amount - (float) $payment->allocated_amount);
        if ($unallocated > 0) {
            DB::table('clients')->where('id', $payment->client_id)->increment('credit_balance', $unallocated);
        }
        if ($payment->order_id) {
            $order = DB::table('orders')->lockForUpdate()->find($payment->order_id);
            DB::table('orders')->where('id', $order->id)->update(['deposit' => (float) $order->deposit + (float) $payment->allocated_amount, 'balance_due' => max(0, (float) $order->balance_due - (float) $payment->allocated_amount), 'updated_at' => now()]);
        }
        if ($payment->invoice_id) {
            $invoice = DB::table('invoices')->lockForUpdate()->find($payment->invoice_id);
            $paid = (float) $invoice->paid_amount + (float) $payment->allocated_amount;
            DB::table('invoices')->where('id', $invoice->id)->update(['paid_amount' => $paid, 'balance_due' => max(0, (float) $invoice->subtotal - $paid), 'status' => $paid >= (float) $invoice->subtotal ? 'payee' : 'partielle', 'updated_at' => now()]);
        }
    }
    ...
    {
        $oldProduct = DB::table('inventory_products')->lockForUpdate()->find($old->inventory_product_id);
        $restored = (float) $oldProduct->quantity + (float) $old->quantity;
        DB::table('inventory_products')->where('id', $oldProduct->id)->update(['quantity' => $restored, 'available_quantity' => max(0, $restored - (float) $oldProduct->reserved_quantity), 'total_purchase_cost' => $restored * (float) $oldProduct->purchase_price + (float) $oldProduct->freight, 'sale_total' => $restored * (float) $oldProduct->sale_price, 'stock_value' => $restored * (float) $oldProduct->purchase_price, 'updated_at' => now()]);
        $product = DB::table('inventory_products')->lockForUpdate()->find($data['inventory_product_id']);
        $quantity = (float) $data['quantity'];
        if ($quantity > (float) $product->available_quantity) {
            throw ValidationException::withMessages(['quantity' => 'Stock disponible insuffisant pour corriger cette vente.']);
        } $total = $quantity * (float) $data['unit_price'];
        $paid = (float) ($data['paid_amount'] ?? 0);
        if ($paid > $total) {
            throw ValidationException::withMessages(['paid_amount' => 'Le paiement ne peut pas dépasser le total.']);
        } if ($paid < $total && (empty($data['buyer_name']) || empty($data['buyer_contact']))) {
            throw ValidationException::withMessages(['buyer_name' => 'Nom et contact requis pour une vente non soldée.']);
        } $after = (float) $product->quantity - $quantity;
        DB::table('inventory_products')->where('id', $product->id)->update(['quantity' => $after, 'available_quantity' => max(0, $after - (float) $product->reserved_quantity), 'total_purchase_cost' => $after * (float) $product->purchase_price + (float) $product->freight, 'sale_total' => $after * (float) $product->sale_price, 'stock_value' => $after * (float) $product->purchase_price, 'exited_at' => today(), 'updated_at' => now()]);
        DB::table('local_sales')->where('id', $id)->update([...$data, 'total' => $total, 'balance_due' => $total - $paid, 'status' => $paid >= $total ? 'paye' : ($paid > 0 ? 'partiel' : 'credit'), 'updated_at' => now()]);
        DB::table('stock_movements')->insert(['inventory_product_id' => $product->id, 'type' => 'inventaire', 'quantity' => $quantity, 'before_quantity' => $product->quantity, 'after_quantity' => $after, 'notes' => 'Correction vente locale', 'moved_at' => now(), 'user_id' => $userId]);
    }
    ...
    private function decorate(array $rows): array
    {
        ...
        $lookups = [
            ...
            'inventory_product_id' => ['inventory_products', 'name'],
            'employee_id' => ['employees', 'name'],
            'user_id' => ['users', 'name'],
            'assigned_to' => ['users', 'name'],
            'quote_id' => ['quotes', 'number'],
        ];
        $maps = [];
        foreach ($lookups as $key => [$table,$label]) {
            $ids = collect($rows)->pluck($key)->filter(fn ($id) => $id !== null && $id !== '')->unique()->values();
            if ($ids->isEmpty()) {
                continue;
            }
            $maps[$key] = DB::table($table)->whereIn('id', $ids)->pluck($label, 'id');
        }

        return array_map(function ($row) use ($maps) {
            if (isset($row['quote_id'])) {
                $row['quote_record_id'] = $row['quote_id'];
            } if (isset($row['order_id'])) {
                $row['order_record_id'] = $row['order_id'];
            } foreach ($maps as $key => $map) {
                if (isset($row[$key])) {
                    $row[$key] = $map[$row[$key]] ?? $row[$key];
                }
            } if (isset($row['order_record_id'])) {
                $row['order_id'] = $row['order_record_id'].'::'.$row['order_id'];
            } if (array_key_exists('proof_path', $row)) {
                $row['proof_path'] = $row['proof_path'] ? '/purchase-proof/'.basename($row['proof_path']) : ($row['proof_url'] ?? null);
            }
    ...
    {
        $select = fn ($name, $label, $options, $required = true, $default = null) => compact('name', 'label', 'options', 'required', 'default') + ['type' => 'select'];
        $multi = fn ($name, $label, $options, $required = false, $default = []) => compact('name', 'label', 'options', 'required', 'default') + ['type' => 'multiselect'];
        $input = fn ($name, $label, $type = 'text', $required = true, $default = null) => compact('name', 'label', 'type', 'required', 'default');
        $clients = in_array($module, ['devis', 'commandes', 'paiements'], true) ? DB::table('clients')->where('active', true)->orderBy('name')->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->name, 'contact' => $x->contact, 'address' => $x->address, 'type' => $x->type, 'credit_balance' => (float) $x->credit_balance])->all() : [];
        $suppliers = $module === 'achats' ? DB::table('suppliers')->where('active', true)->orderBy('name')->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->name, 'contact' => $x->contact])->all() : [];
        $orders = in_array($module, ['paiements', 'factures', 'achats', 'logistique', 'depenses'], true) ? DB::table('orders')->orderByDesc('id')->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->number])->all() : [];
        $quotes = $module === 'commandes' ? DB::table('quotes')->whereNull('deleted_at')->orderByDesc('id')->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->number])->all() : [];
        $products = $module === 'ventes' ? DB::table('inventory_products')->where('quantity', '>', 0)->orderBy('name')->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->name.' ('.$x->quantity.')'])->all() : [];
        $employees = $module === 'salaires' ? DB::table('employees')->where('active', true)->get()->map(fn ($x) => ['value' => $x->id, 'label' => $x->name])->all() : [];
        ...
        if ($module === 'stock') {
            ...
            $originOrders = $sourceOrders->map(function ($order) use ($containers) {
                $references = $containers->get($order->id, collect())->pluck('container_reference')->filter()->unique()->implode(', ');

                return ['value' => $order->id, 'label' => $order->number.($references ? ' · Conteneur '.$references : '')];
            })->all();
        }
        $o = fn (array $values) => array_map(fn ($v) => ['value' => $v, 'label' => ucfirst(str_replace('_', ' ', $v))], $values);
    ...
    {
        if (array_key_exists($module, $this->itemFieldsCache)) {
            return $this->itemFieldsCache[$module];
        }
        if (! in_array($module, ['devis', 'commandes'], true)) {
            return $this->itemFieldsCache[$module] = [];
        }
        ...
        return $this->itemFieldsCache[$module] = [...$base, ['name' => $module === 'devis' ? 'total' : 'client_total', 'label' => 'Prix total (Ar)', 'type' => 'number', 'required' => true, 'readOnly' => $module === 'commandes']];
    }
    ...
    {
        if ($module !== 'commandes' || ! DB::getSchemaBuilder()->hasTable('order_packages')) {
            return [];
        }
        $itemIndexes = DB::table('order_items')->where('order_id', $id)->orderBy('id')->pluck('id')->values()->flip();
        $packages = DB::table('order_packages')->where('order_id', $id)->orderBy('id')->get();
        $packageItems = DB::table('order_package_items')->whereIn('order_package_id', $packages->pluck('id'))->orderBy('id')->get()->groupBy('order_package_id');

        return $packages->map(function ($package) use ($itemIndexes, $packageItems) {
            $items = $packageItems->get($package->id, collect())->map(fn ($line) => ['item_index' => $itemIndexes[$line->order_item_id] ?? 0, 'quantity' => $line->quantity])->all();
    ...
    {
        if (! in_array($module, ['factures', 'logistique'], true)) {
            return [];
        }
    ...
    private function orderTemplates(string $module): array
    {
        ...
        $orders = DB::table('orders')->whereNull('deleted_at')->orderByDesc('id')->get();
        $packageStats = $module === 'logistique'
            ? DB::table('order_packages')->whereIn('order_id', $orders->pluck('id'))->select('order_id', DB::raw('sum(volume_cbm) as volume_cbm'), DB::raw('count(*) as package_count'))->groupBy('order_id')->get()->keyBy('order_id')
            : collect();

        return $orders->map(function ($order) use ($module, $packageStats) {
            if ($module === 'logistique') {
                $stats = $packageStats->get($order->id);

                return ['id' => $order->id, 'number' => $order->number, 'cbm' => (float) ($stats?->volume_cbm ?: $order->cbm), 'package_count' => (int) ($stats?->package_count ?? 0)];
            }
    ...
    private function quoteTemplates(string $module): array
    {
        ...
        $quotes = DB::table('quotes')->leftJoin('clients', 'clients.id', '=', 'quotes.client_id')->whereNull('quotes.deleted_at')->orderByDesc('quotes.id')->get(['quotes.*', 'clients.name as saved_client_name', 'clients.contact as saved_client_contact']);
        $itemsByQuote = DB::table('quote_items')->whereIn('quote_id', $quotes->pluck('id'))->orderBy('id')->get()->groupBy('quote_id');

        return $quotes->map(function ($quote) use ($itemsByQuote) {
            $items = $itemsByQuote->get($quote->id, collect())->map(function ($item) {
            ...
            return ['id' => $quote->id, 'number' => $quote->number, 'client_id' => $quote->client_id, 'client_name' => $quote->client_name ?: $quote->saved_client_name, 'client_contact' => $quote->contact ?: $quote->saved_client_contact, 'shipping_mode' => $quote->shipping_mode, 'items' => $items];
        })->keyBy('id')->all();
    }
    ...
    {
        $config = $this->config($module);
        abort_unless(DB::getSchemaBuilder()->hasTable($config['table']), 404);
        ...
        if ($module === 'devis') {
            [$columns,$rows] = $this->quoteExportData($rows);
        }
        $contents = Excel::raw(new StyledModuleExport($module, $config['title'], $columns, $rows), ExcelFormat::XLSX);
        $filename = $module.'-'.now()->format('Y-m-d-His').'.xlsx';
        $path = $this->storage->putExport($filename, $contents);
    ...
    private function quoteExportData(array $quotes): array
    {
        ...
        foreach ($quotes as $quote) {
            ...
            if ($quoteItems->isEmpty()) {
                $quoteItems = collect([null]);
            }
            foreach ($quoteItems as $item) {
                $rows[] = [...$quote, 'product_name' => $item?->name, 'photo_path' => $item?->photo_path, 'quantity' => $item?->quantity, 'product_total' => $item?->total];
            }
        }
        ...
        if ($search !== '') {
            $query->where(function ($nested) use ($search, $table) {
                ...
                foreach ($columns as $column) {
                    $nested->orWhere($table.'.'.$column, 'like', $term);
                }
                ...
                foreach ($relations as $foreignKey => [$relatedTable,$relatedColumns]) {
                    if (! DB::getSchemaBuilder()->hasColumn($table, $foreignKey)) {
                        continue;
                    }
                    $nested->orWhereIn($table.'.'.$foreignKey, function ($relation) use ($relatedTable, $relatedColumns, $term) {
                        $relation->select('id')->from($relatedTable)->where(function ($match) use ($relatedTable, $relatedColumns, $term) {
                            foreach ($relatedColumns as $column) {
                                $match->orWhere($relatedTable.'.'.$column, 'like', $term);
                            }
                        });
                    });
                }
                    ...
                    $nested->orWhereIn($table.'.id', function ($child) use ($childTable, $foreignKey, $childColumns, $term) {
                        $child->select($foreignKey)->from($childTable)->where(function ($match) use ($childTable, $childColumns, $term) {
                            foreach ($childColumns as $column) {
                                $match->orWhere($childTable.'.'.$column, 'like', $term);
                            }
                        });
                    });
                }
    ...
    private function filterValueLabel(string $module, string $field, mixed $value): string
    {
        ...
        if ($module === 'demandes-devis' && $field === 'status') {
            return [
                'nouveau' => '🆕 Nouveau',
        ...
        if ($module === 'demandes-devis' && $field === 'sourcing_priority') {
            return [
                'urgent' => '🔴 Urgent',
    ...
    private function uniqueProductSlug(string $value, ?int $ignoreId = null): string
    {
        ...
        $slug = $base;
        $suffix = 2;
        while (DB::table('inventory_products')->where('slug', $slug)->when($ignoreId, fn ($query) => $query->where('id', '!=', $ignoreId))->exists()) {
            $slug = $base.'-'.$suffix++;
        }
    ...
    private function auditJson(mixed $value): ?string
    {
        ...
        $normalize = function (mixed $item) use (&$normalize): mixed {
            ...
            if (is_object($item)) {
                return array_map($normalize, get_object_vars($item));
            }
            if (is_resource($item)) {
                return '[resource]';
            }
