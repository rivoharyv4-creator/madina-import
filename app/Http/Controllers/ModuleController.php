...
class ModuleController extends Controller
{
    ...
    private function config(string $module): array
    {
        return [
            ...
            'devis'=>['title'=>'Devis','table'=>'quotes','primary'=>'Nouveau devis','editable'=>true,'columns'=>['number'=>'N° devis','client_id'=>'Client','total'=>'Montant','valid_until'=>'Validité','status'=>'Statut']],
    ...
    public function create(string $module)
    {
        ...
        return Inertia::render('Module/Form',['module'=>$module,'config'=>$config,'fields'=>$this->fields($module),'itemFields'=>$this->itemFields($module),'initialItems'=>$this->itemFields($module)?[$this->emptyItem($module)]:[],'initialPackages'=>[],'initialSupplierProducts'=>$module==='fournisseurs'?[]:null,'orderProducts'=>$this->orderProducts($module)]);
    }
    ...
    public function show(string $module, int $id)
    {
        ...
        $items=DB::table('order_items')
            ...
            ->select('order_items.*','suppliers.name as supplier_name')
            ->get()
    ...
    private function createQuote(array $data, NumberSequenceService $numbers): int
    {
        ...
        $id=DB::table('quotes')->insertGetId(['number'=>$numbers->next('quote'),'client_id'=>$client->id,'contact'=>$client->contact,'client_type'=>$client->type,'sent_at'=>$data['status']==='brouillon'?null:today(),'valid_until'=>$data['valid_until'],'status'=>$data['status'],'supplier_estimate'=>$supplierTotal,'logistics_estimate'=>$logistics,'margin'=>$margin,'total'=>$total,'currency'=>'MGA','notes'=>null,'created_at'=>now(),'updated_at'=>now()]);
    ...
    private function createOrder(array $data, NumberSequenceService $numbers, int $managerId): int
    {
        ...
        $id=DB::table('orders')->insertGetId(['number'=>$numbers->next('order'),'client_id'=>$data['client_id'],'quote_id'=>null,'manager_id'=>$managerId,'origin'=>'directe','ordered_at'=>$data['ordered_at'],'shipping_mode'=>$data['shipping_mode']??null,'cbm'=>$cbm,'freight'=>$freight,'supplier_total'=>$supplierTotal,'commission_enabled'=>$commissionEnabled,'commission_base'=>$supplierTotal,'commission_rate'=>$data['commission_rate']??8,'commission_amount'=>$commission,'margin'=>$margin,'client_total'=>$total,'deposit'=>$deposit,'balance_due'=>$total-$deposit,'status'=>$data['status'],'notes'=>$data['notes']??null,'created_at'=>now(),'updated_at'=>now()]);
    ...
    {
        $values=(array)$record;
        if($module==='devis') { $item=DB::table('quote_items')->where('quote_id',$record->id)->first(); if($item) $values=[...$values,'product_name'=>$item->name,'specifications'=>$item->specifications,'quantity'=>$item->quantity,'supplier_id'=>$item->supplier_id,'commission'=>$item->commission]; }
    ...
    private function applyUpdate(string $module, int $id, object $old, array $data, int $userId, BusinessCalculator $calculator): void
    {
        ...
        if($module==='devis') {
            $client=DB::table('clients')->find($data['client_id']); $supplierTotal=collect($data['items'])->sum(fn($item)=>(float)$item['supplier_price']*(float)$item['quantity']); $logistics=collect($data['items'])->sum(fn($item)=>(float)($item['china_delivery']??0)+(float)($item['packaging']??0)+(float)($item['freight']??0)); $margin=collect($data['items'])->sum(fn($item)=>(float)($item['margin']??0)); $total=collect($data['items'])->sum(fn($item)=>(float)$item['total']);
            DB::table('quotes')->where('id',$id)->update(['client_id'=>$client->id,'contact'=>$client->contact,'client_type'=>$client->type,'sent_at'=>$data['status']==='brouillon'?null:($old->sent_at??today()),'valid_until'=>$data['valid_until'],'status'=>$data['status'],'supplier_estimate'=>$supplierTotal,'logistics_estimate'=>$logistics,'margin'=>$margin,'total'=>$total,'updated_at'=>now()]);
        ...
        if($module==='commandes') {
            ...
            DB::table('orders')->where('id',$id)->update(['client_id'=>$data['client_id'],'ordered_at'=>$data['ordered_at'],'shipping_mode'=>$data['shipping_mode']??null,'cbm'=>$cbm,'freight'=>$freight,'supplier_total'=>$supplierTotal,'commission_enabled'=>$enabled,'commission_base'=>$supplierTotal,'commission_rate'=>$data['commission_rate']??8,'commission_amount'=>$commission,'margin'=>$margin,'client_total'=>$total,'deposit'=>$deposit,'balance_due'=>$total-$deposit,'status'=>$data['status'],'notes'=>$data['notes']??null,'updated_at'=>now()]);
    ...
    private function fields(string $module): array
    {
        ...
        $clients=DB::table('clients')->where('active',true)->orderBy('name')->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->name])->all(); $suppliers=DB::table('suppliers')->where('active',true)->orderBy('name')->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->name])->all(); $orders=DB::table('orders')->orderByDesc('id')->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->number])->all(); $products=DB::table('inventory_products')->where('quantity','>',0)->orderBy('name')->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->name.' ('.$x->quantity.')'])->all(); $employees=DB::table('employees')->where('active',true)->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->name])->all(); $invoices=DB::table('invoices')->orderByDesc('id')->get()->map(fn($x)=>['value'=>$x->id,'label'=>$x->number])->all();
        ...
        return match($module){
            ...
            'devis'=>[$select('client_id','Client',$clients),$input('valid_until','Valide jusqu’au','date'),$select('status','Statut',$o(['brouillon','envoye','negociation','accepte']),true,'brouillon')],
            'commandes'=>[$select('client_id','Client',$clients),$select('commission_enabled','Appliquer une commission',[['value'=>0,'label'=>'Non'],['value'=>1,'label'=>'Oui']],true,0),$input('commission_rate','Taux commission (%)','number',false,8),$input('deposit','Acompte reçu (Ar)','number',false,0),$input('ordered_at','Date de commande','date',true,$today),$select('shipping_mode','Mode d’envoi',$o(['aerien','maritime']),false),$select('status','Statut',$o(['brouillon','demande_recue','attente_validation','confirmee','fonds_recus','achat_en_cours']),true,'brouillon'),$input('notes','Notes internes','textarea',false)],
    ...
    {
        if($module==='devis') return DB::table('quote_items')->where('quote_id',$id)->orderBy('id')->get()->map(fn($item)=>['id'=>$item->id,'name'=>$item->name,'specifications'=>$item->specifications,'quantity'=>$item->quantity,'supplier_id'=>$item->supplier_id,'source_url'=>$item->source_url,'supplier_price'=>$item->supplier_price,'china_delivery'=>$item->china_delivery,'packaging'=>$item->packaging,'weight'=>$item->estimated_weight,'cbm'=>$item->estimated_cbm,'freight'=>$item->estimated_freight,'margin'=>$item->margin,'commission'=>$item->commission,'total'=>$item->total])->all();
        if($module==='commandes') return DB::table('order_items')->where('order_id',$id)->orderBy('id')->get()->map(fn($item)=>['id'=>$item->id,'name'=>$item->name,'photo'=>null,'photo_url'=>$item->photo_path?'/product-photo/'.basename($item->photo_path):null,'specifications'=>$item->specifications,'quantity'=>$item->quantity,'supplier_id'=>$item->supplier_id,'source_url'=>$item->source_url,'supplier_price'=>$item->supplier_price,'china_delivery'=>$item->china_delivery,'packaging'=>$item->packaging,'weight'=>$item->weight,'cbm'=>$item->cbm,'freight'=>$item->freight,'margin'=>$item->margin,'commission'=>$item->commission,'client_total'=>$item->client_total])->all();
        return [];
    }
