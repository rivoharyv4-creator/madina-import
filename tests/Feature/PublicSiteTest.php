...
class PublicSiteTest extends TestCase
{
    ...
    public function test_visitors_use_public_login_without_exposing_private_login(): void
    {
        ...
        $this->get('/dashboard')->assertRedirect(route('customer.login', absolute: false));
    ...
    public function test_contact_form_is_validated_and_saved_for_management(): void
    {
        ...
        $this->actingAs($manager)->get('/modules/demandes')->assertInertia(fn (Assert $page) => $page
            ->where('notifications.publicRequests', 1)
        );
        $this->actingAs($manager)->get("/modules/demandes/{$requestId}")->assertOk()->assertInertia(fn (Assert $page) => $page
            ...
            ->where('request.status', 'lue')
            ->where('notifications.publicRequests', 0)
        );
        $this->assertDatabaseHas('contact_requests', ['id' => $requestId, 'status' => 'lue']);
