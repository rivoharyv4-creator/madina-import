...
Route::get('/dashboard', DashboardController::class)->middleware(['auth', 'backoffice.account', 'verified', 'module.access:dashboard'])->name('dashboard');

Route::middleware(['auth', 'backoffice.account'])->group(function () {
    ...
    Route::middleware('super.admin')->group(function () {
        ...
        Route::get('/admin/utilisateurs/{user}/double-authentification', [UserTwoFactorController::class, 'show'])->name('admin.users.two-factor.show')
            ->missing(function (Request $request) {
                ...
                return redirect()->route('admin.users.index')->with('missing_user', 'Ce compte est introuvable. Sélectionnez un utilisateur dans la liste actualisée.');
            });
    ...
    Route::get('/secure-files/{category}/{filename}', [SecureFileController::class, 'show'])->whereIn('category', ['product', 'payment', 'expense', 'invoice', 'quote', 'logistics', 'delivery'])->where('filename', '[A-Za-z0-9._-]+')->name('secure-files.show');
    Route::get('/modules/bons-livraison', [DeliveryNoteController::class, 'index'])->middleware('module.access:bons-livraison')->name('delivery-notes.index');
    Route::get('/modules/bons-livraison/create', [DeliveryNoteController::class, 'create'])->middleware('module.access:bons-livraison')->name('delivery-notes.create');
    Route::post('/modules/bons-livraison', [DeliveryNoteController::class, 'store'])->middleware('module.access:bons-livraison')->name('delivery-notes.store');
    Route::get('/modules/bons-livraison/{id}/edit', [DeliveryNoteController::class, 'edit'])->middleware('module.access:bons-livraison')->whereNumber('id')->name('delivery-notes.edit');
    Route::put('/modules/bons-livraison/{id}', [DeliveryNoteController::class, 'update'])->middleware('module.access:bons-livraison')->whereNumber('id')->name('delivery-notes.update');
    Route::get('/modules/bons-livraison/{id}/pdf', [DeliveryNoteController::class, 'pdf'])->middleware('module.access:bons-livraison')->whereNumber('id')->name('delivery-notes.pdf');
