...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        DB::transaction(function () use ($manager): void {
            ...
            unset($product);
            foreach ($supplierProducts as $product) {
                DB::table('supplier_products')->updateOrInsert(['supplier_id' => $product['supplier_id'], 'name' => $product['name']], $product);
            }
            ...
            unset($item);
            foreach ($orderItems as $item) {
                DB::table('order_items')->updateOrInsert(['order_id' => $item['order_id'], 'name' => $item['name']], $item);
            }
            ...
            unset($payment);
            foreach ($payments as $payment) {
                DB::table('client_payments')->updateOrInsert(['reference' => $payment['reference']], $payment);
            }
            ...
            foreach ($stock as &$product) {
                ...
                $product['cbm'] = null;
                $product['freight'] = 0;
                $product['total_purchase_cost'] = $product['quantity'] * $product['purchase_price'];
                $product['sale_total'] = $product['quantity'] * $product['sale_price'];
                $product['created_at'] = $now;
                $product['updated_at'] = $now;
            }
            unset($product);
            ...
            DB::table('tax_records')->updateOrInsert(['type' => 'impot_synthetique', 'period' => '2026'], ['fiscal_year' => 2026, 'calculation_base' => 'benefice_net', 'base_amount' => 7250000, 'rate' => 5, 'calculated_amount' => 362500, 'declared_amount' => null, 'due_at' => '2027-03-31', 'declared_at' => null, 'paid_at' => null, 'status' => 'estimation', 'created_at' => $now, 'updated_at' => $now]);
